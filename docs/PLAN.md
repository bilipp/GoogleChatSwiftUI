...
| A **forwarded** message arrives in the same `quotedMessageMetadata` field as an inline reply, told apart only by `quoteType: FORWARD`. The field is documented as defaulting to `REPLY` when unset, so absence is not "unknown". A forward's snapshot carries four fields a reply's does not — `formattedText`, `annotations`, `attachments`, and a `forwardedMetadata` naming the source space — because the original may be in a space this account is not a member of | Forwards render as a block of their own beneath the bubble, from the snapshot alone and never from the cache: the snapshot is what was passed on, and preferring a cached original would make the same forward read differently depending on which spaces happen to be open. Its attachments are stored as JSON rather than `CachedAttachment` rows, whose unique resource name would collide with the original's. `quoteType` is kept as a raw string, not an enum — a `RawRepresentable` throws on an unknown value, and the throw would fail the decode of the whole *message* |
...
| Nothing in the API states how a `chat.google.com` URL maps onto resource names — but Chat states it itself: paste a message link into a conversation and it comes back with a `CHAT_SPACE` `richLinkMetadata` carrying the URL *and* the `spaces/…` names it resolved to. `room/{space}/{thread}/{message}`, and a message id is `{thread}.{message}` | Links are handled in-app rather than in a browser, and "Copy Link" produces the same URL the web client's own would. `ChatDeepLink` owns the mapping in both directions, pinned by tests built from those annotations |
| Nothing in the Chat API creates or attaches a video meeting: no field on `Message`, no conference object, and `cardsV2` is unauthorable as a user. What Chat *does* have is the receiving half — a posted `meet.google.com` URL comes back annotated as a `MEET_SPACE` rich link, with its meeting code | Starting a meeting goes through the Meet REST API instead (`spaces.create`, one scope: `meetings.space.created`), and its `meetingUri` is posted as ordinary message text. Chat annotates it on the way back, so the link this app sends renders as the same green "Meet call" chip as one sent from the web client — see `RichLinkChip`. Nothing about the meeting is cached: the message is the record |
| `Space.spaceUri` — "the URI for a user to access the space", which would settle `room` versus `dm` outright — is documented and declared in the discovery document but **absent from every `spaces.list` response** for this account | The word comes from `spaceType`: `dm` for a one-to-one, `room` for a space *and for a group chat*, confirmed against the web client rather than inferred — a group chat is named from its members like a DM and only its id gives it away as a space. `spaceUri` is stored and preferred if it ever arrives |
